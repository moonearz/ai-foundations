...
def matrix_product_improved(m1: list[list[int]], m2: list[list[int]]) -> list[list[int]]:
    ...
    return [
        [sum(a * b for a, b in zip(row, col)) for col in m2_t]
        for row in m1
    ]
...
for A, B in invalid_cases:
    assert matrix_product(A, B) == None
